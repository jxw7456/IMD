...
namespace Webster_MonoGame_Vectors
{
    class ShapeDrawer
    {
        ...
        public void DrawLine(int x0, int y0, int x1, int y1, int thickness, Color color)
        {
            ...
            Rectangle rectToDraw = new Rectangle(x0, y0, (int)Math.Ceiling(length), thickness);

            //Use the Draw() overload that accepts a rotation
            spriteBatch.Draw(pixel, rectToDraw, null, color, angle, new Vector2(0, 0.5f), SpriteEffects.None, 0.0f);
        }
